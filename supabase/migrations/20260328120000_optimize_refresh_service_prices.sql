-- Speed up refresh_service_prices: materialize vw_raw_prices_parsed once per run,
-- optional incremental refresh by crawl_batch_id, indexes for offers lookups.
-- Apply in Supabase: SQL Editor (paste) or `supabase db push`.

CREATE INDEX IF NOT EXISTS idx_raw_prices_crawl_batch_id
  ON public.raw_prices (crawl_batch_id);

-- Speeds up NOT EXISTS (product_id, store_id, collected_at, final_url) in STEP 4.
...
BEGIN
  ------------------------------------------------------------------
  -- Materialize parsed rows once (avoids scanning vw_raw_prices_parsed 3×).
  -- p_crawl_batch_id NULL = full refresh; set = only rows from that crawl.
  ------------------------------------------------------------------
  ...
  ------------------------------------------------------------------
  -- STEP 4-A. 기존 raw_price_id row update
  ------------------------------------------------------------------
...
COMMENT ON FUNCTION public.refresh_service_prices(text) IS
  'Refreshes stores/products/offers from vw_raw_prices_parsed. Pass crawl_batch_id after insert for incremental run; NULL = all rows.';
